export function drawChoropleth1(svg) {
  d3.json("data/INDIA_STATES_simplify.geojson").then(geoData => {
    // Set up color scale
    const color = d3.scaleSequential()
      .domain([0, d3.max(geoData.features, d => +d.properties.num_caaqms)])
    ...
    svg.selectAll("path")
      ...
      .attr("fill", d => {
        const count = d.properties.num_caaqms;
    ...
    svg.selectAll("text.state-label")
        ...
        .text(d => d.properties.num_caaqms)
        .attr("text-anchor", "middle")
        .attr("font-size", "1rem")
        .attr("font-weight", "bold")
        .attr("fill", d => {
            const fillColor = color(d.properties.num_caaqms || 0);
      ...
      .on("mouseover", (event, d) => {
        ...
        tooltip.html(
          `<strong>${d.properties.stname}</strong><br/>
           Monitors: ${d.properties.num_caaqms}`
        )
